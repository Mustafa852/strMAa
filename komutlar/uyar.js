...
exports.run = (client, message, args) => {
  ...
  if (db.has(`codeminglog_${message.guild.id}`) === false) return message.reply('Mod log kanalı ayarlanmamış');
  let modlog = message.guild.channels.get(db.fetch(`codeminglog_${message.guild.id}`).replace("<#", "").replace(">", ""));
